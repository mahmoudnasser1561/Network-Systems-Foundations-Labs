...
def server_program():
    ...
    while True:
        ...
        try:
            JsonRequest = recv_message(conn)
            Obj = make_call(JsonRequest)
            # pass the received message to make_call(json_dict)
            send_message(conn, {"status": "OK", "response": Obj})
        except:
